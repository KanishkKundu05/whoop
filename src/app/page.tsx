...
export default async function Home({ searchParams }: { searchParams: Promise<{ auth_error?: string }> }) {
  ...
  return <OnboardingShell wide>
    ...
    <div className="grid gap-5 md:grid-cols-3">
      {[{ name: "Garmin", Icon: Watch, description: "A new rhythm for your training." }, { name: "Apple Watch", Icon: Apple, description: "More from the watch you wear every day." }].map(({ name, Icon, description }) => <div key={name} aria-disabled="true" className="flex min-h-52 flex-col rounded-3xl border md:min-h-72 border-zinc-200 bg-white/60 p-7">
      ...
      <Link href={connected ? "/whoop" : "/setup/connection"} prefetch={false} className="group order-first flex min-h-72 flex-col md:order-last rounded-3xl bg-zinc-950 p-7 text-white shadow-lg shadow-zinc-950/10 transition hover:-translate-y-1 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-lime-600 motion-reduce:transform-none">
