  ...
  return <OnboardingShell>
    {connected ? <>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2">
        {[{ href: "/whoop/music", Icon: Headphones, title: "Sync music to your pace", description: "at the end of song A, we check your heart rate and play song B of a similar BPM", cta: "Find your rhythm" }, { href: "/setup", Icon: Moon, title: "morning sleep report", description: "Uses a whoop webhook to send ur sleep report to someone you care about when you wake up", cta: "Set up morning texts" }].map(({ href, Icon, title, description, cta }) => <Link key={href} href={href} prefetch={false} className="group flex flex-col rounded-3xl border border-zinc-200 bg-white p-7 transition hover:border-lime-500 hover:shadow-sm focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-lime-600">
      ...
      <Link href="/setup/connection" className="mt-6 inline-flex min-h-11 items-center gap-2 text-sm text-zinc-600 underline underline-offset-4">Your sleep and account<ArrowRight size={15} /></Link>
      ...
      {config.isReady ? <Link href="/setup/connection" className={`${primaryAction} mt-7`}>Connect WHOOP<ArrowRight size={18} /></Link> : <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900"><p>WHOOP connection isn’t available yet. Please try again later.</p></div>}
