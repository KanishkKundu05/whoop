...
export default async function WhoopPage({ searchParams }: { searchParams: Promise<{ auth_error?: string }> }) {
  ...
  return <OnboardingShell>
    {connected ? <>
      ...
      {config.isReady ? <a href="/api/auth/whoop?next=/whoop" className={`${primaryAction} mt-7`}>Connect WHOOP<ArrowRight size={18} /></a> : <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm leading-6 text-amber-900"><p>WHOOP connection isn’t available yet. The app owner needs to finish configuration.</p><Link href="/setup/connection" className="mt-3 inline-block underline underline-offset-4">Open connection settings</Link></div>}
